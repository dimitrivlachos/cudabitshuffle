...
template <typename... T> auto bold(T... args) -> std::string {
  return with_formatting("\033[1m", args...);
}
template <typename... T> auto blue(T... args) -> std::string {
  return with_formatting("\033[34m", args...);
}
template <typename... T> auto red(T... args) -> std::string {
  return with_formatting("\033[31m", args...);
}
template <typename... T> auto green(T... args) -> std::string {
  return with_formatting("\033[32m", args...);
}
template <typename... T> auto gray(T... args) -> std::string {
  return with_formatting("\033[37m", args...);
}
template <typename... T> auto yellow(T... args) -> std::string {
...
template <typename T>
void draw_image_data(const T *data, size_t fast, size_t slow, size_t width,
                     size_t height, size_t data_width, size_t data_height) {
...
void draw_image_data(const std::unique_ptr<T, U> &data, size_t fast,
                     size_t slow, size_t width, size_t height,
                     size_t data_width, size_t data_height) {
  draw_image_data(static_cast<T *>(data.get()), fast, slow, width, height,
                  data_width, data_height);
}
template <typename T>
void draw_image_data(const span<T> data, size_t fast, size_t slow, size_t width,
                     size_t height, size_t data_width, size_t data_height) {
  draw_image_data(data.data(), fast, slow, width, height, data_width,
                  data_height);
}
...
template <typename T, typename U>
bool compare_results(const T *left, const size_t left_pitch, const U *right,
                     const size_t right_pitch, std::size_t width,
                     std::size_t height, size_t *mismatch_x = nullptr,
